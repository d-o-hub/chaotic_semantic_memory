...
### Changed
- **Retrieval (`csm-memory`)**: bucketed candidate generation no longer loses recall as the corpus grows. The probe width now scales with N (`effective_bucket_probe_width`; the configured `bucket_probe_width` is a floor), candidates within one masked bit of the query bucket are accepted, and a probe that exceeds `max_candidates × 2` returns nothing so the caller performs the exact scan — previously the oversized bucket was truncated by index order, which measured recall@10 **0.016** at 200 k with the default floor 2. Measured after the fix: at floor 8, 570 candidates / recall **0.832** / 249 µs at 10 k (exact scan 558 µs); at 50 k and 200 k the probe declines and the exact scan answers (recall 1.000). Sweep evidence in `plans/evidence/scale_release_2026_09_21/bucket_sweep/`.
- **Evidence (Tier 3, ADR-0095)**: `plans/REFERENCE_RUNNER.md` names the reference profile (`csm-ref-01`) and the rules for publishing numbers; `scripts/bench-baseline.sh` records `plans/evidence/bench/canonical.json` (88 Criterion benchmarks with median **and confidence interval**, commit, toolchain, CPU) and compares against it, failing only when a delta exceeds the tolerance *and* the intervals do not overlap (`--advisory` for laptop runs; `pre-release-validate.sh` now uses this instead of the `--baseline main` that could never resolve); `scripts/wasm-evidence.sh` records the npm package bytes, SHA-256s and the Node smoke transcript in `plans/evidence/wasm_<date>/`; release-scale artifacts land in `plans/evidence/scale_release_2026_09_21/`. Flag `benchmarks_prove_performance` -> true.
- **Retrieval (`csm-memory`)**: the release-scale evidence found bucketed candidate recall@10 collapsing with corpus size (0.604 at 10k → 0.208 at 200k) because `bucket_probe_width` is fixed; queued as `fix_bucketed_candidate_recall_at_scale`. HNSW recall at fixed `ef_search` also falls (0.908 → 0.800 by 200k) and is documented in the artifact.
- **WASM artifact parity**: CI built `--dev --target nodejs` (4 077 828 B) while `release.yml` published `--release --target web` with wasm-opt (656 657 B) — a 6.2x divergence, and `scripts/wasm_size_gate.sh` guarded a third artifact (a raw `cargo build` output, 1 133 971 B) that nobody ships. All four consumers (CI, release, size gate, TS-freshness) now go through `scripts/build-wasm.sh <dev-nodejs|dev-web|release-web>`; CI builds and smoke-tests the release/web package it publishes, uploads it as an artifact, and the size gate measures an existing package directory (`release-web` output) or builds one, threshold re-based to 800 000 B from the measured 656 657 B, and `scripts/validate.sh` only runs it where wasm-pack exists (the CI wasm job passes the package it just built). `wasm/test.js` can now load either target (it hands the wasm bytes to the web glue's init, since Node's `fetch` rejects `file://`).
### Changed
- **Dedup (`csm-core-lib`)**: `maps::neural_circuit` is now a re-export of the `csm-chaos` owner instead of a second, unreachable copy of the same map (the `experimental-neural-circuit` feature was enabled by nobody in the workspace). The feature forwards to `csm-chaos/experimental-neural-circuit` and keeps the previous serde capability via `csm-chaos/serde`; trajectories come from the owner's `libm::tanh` implementation.
- **Tests**: removed 24 test bodies that were byte-identical duplicates of owner-crate tests (12 in `src/embedding/mod.rs`, 6 in `src/persistence_wasm.rs` — a wasm32-only module whose test module could not even compile, 3 in `crates/csm-core-lib/src/maps/neural_circuit.rs`, and one each in `src/lib.rs`, `src/export_payload/export_payload_tests.rs`, `src/wasm_ext_tests.rs`), plus an empty test module in `src/wasm_graph_rag.rs`. Root facade test attributes 196 → 175, owner crates 299 → 296, workspace total 1053 → 1029.
- **Fix (`csm-embedding`)**: the env-mutating provider tests now serialize `std::env` through a `static ENV_TEST_LOCK`, matching the root copies; the crate still carried the racy pattern (and a "single-threaded test" SAFETY comment) that PR #700 fixed only in the facade.
- **Coverage methodology (ADR-0095)**: `scripts/update-coverage.sh` (a test-LOC/source-LOC ratio that ignored `crates/` and rewrote README rows that no longer exist) is replaced by `scripts/coverage-report.sh` — `inventory` reports unique compiled behavior per layer, `llvm-cov` reports line and branch coverage for the workspace.
