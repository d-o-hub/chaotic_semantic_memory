    ...
    c.bench_function("delete_concept", |b| {
        b.iter(|| {
            ...
            rt.block_on(async {
                let persistence = Persistence::new_local(path).await.unwrap();
                persistence
                    .save_concept(NS, &make_concept("to-delete"))
                    .await
                    .unwrap();
                persistence
                    .delete_concept(NS, black_box("to-delete"))
                    .await
                    .unwrap();
                black_box(persistence)
            })
    ...
    c.bench_function("delete_concept_with_cascade", |b| {
        b.iter(|| {
            ...
            rt.block_on(async {
                ...
                for i in 0..9 {
                    persistence
                        .save_association(NS, "cascade-0", &format!("cascade-{i}"), 0.5)
                        .await
                        .unwrap();
                }
                persistence
                    .delete_concept(NS, black_box("cascade-0"))
                    .await
                    .unwrap();
                black_box(persistence)
            })
        ...
        group.bench_function(format!("{size}_concepts"), |b| {
            b.iter(|| {
                ...
                rt.block_on(async {
                    let persistence = Persistence::new_local(path).await.unwrap();
                    persistence
                        .save_concepts(NS, black_box(&concepts))
                        .await
                        .unwrap();
                    black_box(persistence)
                })
    ...
    c.bench_function("save_association", |b| {
        b.iter(|| {
            ...
            rt.block_on(async {
                let persistence = Persistence::new_local(path).await.unwrap();
                persistence
                    .save_concept(NS, &make_concept("assoc-from"))
                    .await
                    .unwrap();
                persistence
                    .save_concept(NS, &make_concept("assoc-to"))
                    .await
                    .unwrap();
                persistence
                    .save_association(
        ...
        group.bench_function(format!("{assoc_count}_associations"), |b| {
            b.iter(|| {
                ...
                rt.block_on(async {
                    let persistence = Persistence::new_local(path).await.unwrap();
                    persistence
                        .save_concept(NS, &make_concept("hub"))
                        .await
                        .unwrap();
                    for i in 0..assoc_count {
                        persistence
                            .save_concept(NS, &make_concept(&format!("spoke-{i}")))
                            .await
                            .unwrap();
                    }
                    for i in 0..assoc_count {
                        persistence
                            .save_association(NS, "hub", &format!("spoke-{i}"), 0.5)
                            .await
                            .unwrap();
                    }
                    let associations = persistence
                        .load_associations(NS, black_box("hub"))
                        .await
                        .unwrap();
        ...
        b.iter(|| {
            ...
            rt.block_on(async {
                ...
                let gone = persistence
                    .load_concept::<HVec10240>(NS, "roundtrip")
                    .await
                    .unwrap();
                black_box(gone)
            })
    ...
    c.bench_function("crud_roundtrip_with_associations", |b| {
        b.iter(|| {
            ...
            rt.block_on(async {
                ...
                persistence
                    .save_association(NS, "rt-0", "rt-1", 0.8)
                    .await
                    .unwrap();
                persistence
                    .save_association(NS, "rt-0", "rt-2", 0.6)
                    .await
                    .unwrap();
                persistence
                    .save_association(NS, "rt-1", "rt-3", 0.4)
                    .await
                    .unwrap();

                let loaded = persistence
                    .load_concept::<HVec10240>(NS, "rt-0")
                    .await
                    .unwrap()
                    .unwrap();
                black_box(&loaded);
            ...
            rt.block_on(async {
                ...
                for i in 0..10 {
                    ...
                    handles.push(tokio::spawn(async move {
                        ...
                        loop {
                            match p.save_concept(NS, &concept).await {
                                Ok(_) => break,
