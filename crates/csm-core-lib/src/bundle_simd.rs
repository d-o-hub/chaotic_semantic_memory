...
pub(crate) unsafe fn update_counts_simd_neon(
    ...
    // SAFETY: sel_low/sel_high point to valid static 4-element u32 arrays.
    ...
    for i in 0..80 {
        ...
        for j in 0..16 {
            ...
            // SAFETY: counts_ptr + j * 8 is within counts bounds.
            unsafe {
                let target_ptr_low = counts_ptr.add(j * 8);
            ...
            // SAFETY: counts_ptr + j * 8 + 4 is within counts bounds.
            unsafe {
                let target_ptr_high = counts_ptr.add(j * 8 + 4);
