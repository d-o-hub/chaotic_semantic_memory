...
fn bench_slhm2d_next_reexport(c: &mut Criterion) {
    ...
    c.bench_function("slhm2d_next_reexport", |b| b.iter(|| map.next_value()));
}

#[cfg(feature = "chaotic-hashing")]
fn bench_chaotic_lsh_project_reexport(c: &mut Criterion) {
    let input_dim = 128;
    let lsh = ChaoticLsh::new(0.1, 0.2, 0.99, input_dim);
    let input = vec![1.0f32; input_dim];
    c.bench_function("chaotic_lsh_project_128_reexport", |b| {
