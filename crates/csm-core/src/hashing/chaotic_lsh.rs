...
impl ChaoticLsh {
    ...
    unsafe fn project_avx2(&self, input: &[f32]) -> BHVec10240 {
        use std::arch::x86_64::*;
        ...
        for i in 0..10240 {
            ...
            for c in 0..chunks {
                ...
                let b = unsafe { _mm256_loadu_ps(self.projection_matrix.as_ptr().add(base)) };
    ...
    #[cfg(target_arch = "aarch64")]
    #[inline]
    unsafe fn project_neon(&self, input: &[f32]) -> BHVec10240 {
        ...
        for i in 0..10240 {
            ...
            for c in 0..chunks {
                ...
                let a = unsafe { vld1q_f32(input.as_ptr().add(c * 4)) };
                let b = unsafe { vld1q_f32(self.projection_matrix.as_ptr().add(base)) };
