...
impl<H: Hypervector> LshIndex<H> {
    ...
    /// Compute table hash directly from a pre-serialized hypervector byte slice.
    ///
    /// Memory Optimization: Accepts `&[u8]` slice rather than `&H` to allow callers to
    /// serialize hypervector bytes once per operation (insert/delete/search), reducing
    /// transient 1280-byte heap allocations from O(T) to O(1) across T hash tables.
