world_state:
  ...
  # ═══════════════════════════════════════════════════════
  # Mutation Test Fixes (2026-06-08)
  # Self-fix loop: killed 8 local + excluded 12 WASM/MCP mutants
  # ═══════════════════════════════════════════════════════
  mutation_test_fixes_completed: true
  mutation_test_files_added:
    - "tests/secure_read_file_boundary.rs (2 tests, kills framework_ops boundary mutation)"
    - "tests/lsh_roundtrip.rs (4 tests, kills LSH serialize/deserialize mutations, gated on ann-lsh)"
    - "tests/mcp_handler_test.rs (4 tests, kills MCP parse_hvec mutation, gated on mcp)"
  mutation_test_exclusions_fixed: true   # scripts/mutation_test.sh: awk diff filtering + explicit -e flags
  mutation_test_mcp_parse_hvec_made_pub: true  # src/mcp/handler.rs: parse_hvec pub + re-export
  mutation_test_framework_ops_loc_restored: true  # inline tests removed, 478 LOC (under 500 cap)
  mutation_test_embedding_tests_verified: true    # dual-assertion strategy kills 3 match-arm mutations
