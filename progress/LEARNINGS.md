...
## 2026-04-21: GOAP State Audit & Plan Hygiene

### What Worked
1. Cross-referencing GOAP_STATE.md claims against actual source code (grep for implementations)
2. Checking ACTIONS.md statuses against implementation reality
3. Identifying duplicate YAML keys that cause silent parser overwrites

### Technical Insights
- YAML duplicate keys are silently resolved (last wins) — causes state drift between what humans read and what parsers see
- ACTIONS.md at 3158 lines exceeds the 64KB read limit for agent tools — needs splitting
- Research features (InertialESN, selectivity-aware retrieval) were implemented but never had dedicated test files created
- LEARNINGS.md can accumulate exact duplicate sections from multiple agent sessions writing the same content

### What to Avoid
- Do not mark GOAP_STATE features as `tested: true` without verifying test files exist in `tests/`
- Do not let plan files grow unbounded — archive completed phases periodically
