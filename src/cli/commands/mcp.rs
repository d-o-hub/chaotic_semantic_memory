...
pub async fn run_mcp(args: McpArgs, db_path: Option<&Path>) -> Result<()> {
    match args.command {
        McpCommands::Serve(serve_args) => {
            #[cfg(feature = "mcp")]
            {
                let framework = super::create_framework(db_path).await?;
                crate::mcp::serve(framework, serve_args.transport, &serve_args.bind)
                    .await
