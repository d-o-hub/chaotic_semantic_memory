...
impl BHVec10240 {
    ...
    pub fn to_f32(&self) -> HVec10240 {
        ...
        for i in 0..10240 {
            if (self.bits[i/64] & (1u64 << (i%64))) != 0 { data[i] = 1.0; } else { data[i] = -1.0; }
        }
        HVec10240 { data }
    }
}
...
impl Hypervector for BHVec10240 {
    ...
    fn bind(&self, other: &Self) -> Self {
        let mut bits = [0u64; 160];
        for i in 0..160 { bits[i] = self.bits[i] ^ other.bits[i]; }
        Self { bits }
    }
    fn bundle(vectors: &[Self]) -> Result<Self> {
        ...
        for v in vectors {
            for i in 0..10240 {
                if (v.bits[i/64] & (1u64 << (i%64))) != 0 { counts[i] += 1; } else { counts[i] -= 1; }
            }
        }
        let mut bits = [0u64; 160];
        for i in 0..10240 {
            if counts[i] > 0 { bits[i/64] |= 1u64 << (i%64); }
        }
        Ok(Self { bits })
    }
