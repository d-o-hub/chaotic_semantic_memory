...
pub(crate) unsafe fn hamming_distance_simd_avx2(lhs: &[u128; 80], rhs: &[u128; 80]) -> u32 {
    ...
    for i in (0..80).step_by(2) {
        ...
        total_count = _mm256_add_epi64(total_count, _mm256_sad_epu8(combined, _mm256_setzero_si256()));
    }
    let mut out = [0u64; 4];
...
pub(crate) unsafe fn hamming_distance_simd_neon(lhs: &[u128; 80], rhs: &[u128; 80]) -> u32 {
    ...
    for i in 0..80 {
        let (a, b) = unsafe { (vld1q_u8(lhs.as_ptr().add(i).cast()), vld1q_u8(rhs.as_ptr().add(i).cast())) };
        let x = veorq_u8(a, b);
        let pop = vcntq_u8(x);
...
pub(crate) fn bind_simd_x86(lhs: &[u128; 80], rhs: &[u128; 80]) -> [u128; 80] {
    #[cfg(target_arch = "x86")] use std::arch::x86::{__m128i, _mm_loadu_si128, _mm_storeu_si128, _mm_xor_si128};
    #[cfg(target_arch = "x86_64")] use std::arch::x86_64::{__m128i, _mm_loadu_si128, _mm_storeu_si128, _mm_xor_si128};
    let mut out = [0u128; 80];
    for i in 0..80 {
        unsafe {
...
pub(crate) fn and_simd_x86(lhs: &[u128; 80], rhs: &[u128; 80]) -> [u128; 80] {
    #[cfg(target_arch = "x86")] use std::arch::x86::{__m128i, _mm_and_si128, _mm_loadu_si128, _mm_storeu_si128};
    #[cfg(target_arch = "x86_64")] use std::arch::x86_64::{__m128i, _mm_and_si128, _mm_loadu_si128, _mm_storeu_si128};
    let mut out = [0u128; 80];
    for i in 0..80 {
        unsafe {
            let a = _mm_loadu_si128((&lhs[i] as *const u128).cast::<__m128i>());
...
mod tests {
    ...
    #[test]
    fn test_hamming_distance_matches_bit_count() {
        ...
        let expected: u32 = lhs.iter().zip(rhs.iter()).map(|(l, r)| (l ^ r).count_ones()).sum();
