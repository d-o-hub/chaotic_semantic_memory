...
/// NEON-optimized bit-sliced bundling for a single 128-bit block (1 word).
...
pub(crate) unsafe fn bundle_block_neon_single(
    ...
    use std::arch::aarch64::{
        vandq_u8, vbicq_u8, vdupq_n_u8, veorq_u8, vget_lane_u64, vld1q_u8, vorrq_u8,
    ...
    for v in vectors {
        ...
        for plane in planes.iter_mut().take(num_planes) {
            ...
            *plane = veorq_u8(*plane, carry);
            ...
            if vget_lane_u64(c64, 0) == 0 && vget_lane_u64(c64, 1) == 0 {
                break;
            }
        }
    }
    ...
    let mut out = 0u128;
    unsafe { vst1q_u8((&mut out as *mut u128).cast(), res) };
    out
}
...
pub(crate) unsafe fn bundle_block_neon(
    ...
    let mut out = [0u128; 80];
    for i in 0..80 {
        out[i] = unsafe { bundle_block_neon_single(vectors, i, threshold, num_planes) };
    }
    out
}
...
mod tests {
    ...
    fn bundle_block_avx2_correctness() {
        if std::arch::is_x86_feature_detected!("avx2") {
            use crate::hyperdim::HVec10240;
            let vectors: Vec<HVec10240> = (0..10).map(|i| HVec10240::new_seeded(i)).collect();
