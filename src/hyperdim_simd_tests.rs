...
// ── AVX2 hamming distance test helpers ────────────────────────────────
#[cfg(all(not(target_arch = "wasm32"), target_arch = "x86_64"))]
fn assert_avx2_matches_scalar(lhs: &[u128; 80], rhs: &[u128; 80], expected: Option<u32>) {
    if std::arch::is_x86_feature_detected!("avx2") {
        let simd = unsafe { hamming_distance_simd_avx2(lhs, rhs) };
        let scalar = hamming_distance_optimized(lhs, rhs);
        assert_eq!(simd, scalar, "AVX2 vs scalar mismatch");
        if let Some(exp) = expected {
            assert_eq!(simd, exp, "Unexpected hamming distance");
        }
    }
}
...
fn hamming_distance_simd_avx2_correctness() {
    let (lhs, rhs) = make_test_vectors();
    assert_avx2_matches_scalar(&lhs, &rhs, None);
    // Random vectors stress test
    if std::arch::is_x86_feature_detected!("avx2") {
        use crate::hyperdim::HVec10240;
        for i in 0..10 {
            let v1 = HVec10240::new_seeded(i);
            let v2 = HVec10240::new_seeded(i + 100);
            assert_avx2_matches_scalar(&v1.data, &v2.data, None);
        }
    }
}
...
fn hamming_distance_simd_avx2_all_ones_vs_zeros() {
    let lhs = [0xFFFFFFFFFFFFFFFF_FFFFFFFFFFFFFFFFu128; 80];
    let rhs = [0u128; 80];
    assert_avx2_matches_scalar(&lhs, &rhs, Some(10240));
}
...
fn hamming_distance_simd_avx2_identical() {
    let v = [0x123456789ABCDEF_0FEDCBA987654321u128; 80];
    assert_avx2_matches_scalar(&v, &v, Some(0));
}
...
fn hamming_distance_simd_avx2_single_bit() {
    ...
    let rhs = [0u128; 80];
    assert_avx2_matches_scalar(&lhs, &rhs, Some(80));
}
...
fn hamming_distance_simd_avx2_random_100() {
    ...
    // Uses inline comparison (not helper) to preserve iteration counter in error messages.
    if std::arch::is_x86_feature_detected!("avx2") {
        use crate::hyperdim::HVec10240;
        for i in 0..100 {
            let v1 = HVec10240::new_seeded(i);
...
fn hamming_distance_simd_avx2_sparse_pattern() {
    ...
    let rhs = [0u128; 80];
    assert_avx2_matches_scalar(&lhs, &rhs, Some(1));
}
