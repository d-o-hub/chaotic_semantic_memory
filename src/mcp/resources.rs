...
use rmcp::model::{Resource, ErrorData, ResourceContents, Annotated, RawResource, ResourceTemplate, RawResourceTemplate, ReadResourceResult};
...
pub async fn read_resource(framework: &ChaoticSemanticFramework, uri: &str) -> Result<ReadResourceResult, ErrorData> {
    ...
    if let Some(id) = uri.strip_prefix("concept://") {
        ...
        if let Some(c) = concept {
            ...
            return Err(ErrorData::invalid_params(format!("Concept not found: {}", id), None));
        }
    }

    Err(ErrorData::invalid_params(format!("Unsupported URI: {}", uri), None))
}
