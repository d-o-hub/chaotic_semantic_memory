...
impl Persistence {
    ...
    pub async fn association_count(&self, ns: &str) -> Result<usize> {
        ...
        let mut rows = conn
            .query(
                "SELECT COUNT(*) FROM csm_associations WHERE namespace = ?1",
                params![ns],
            )
            .await
            .map_err(|e| MemoryError::database(format!("Failed to count associations: {e}")))?;
    ...
    pub async fn for_each_association_scoped<F, Fut>(&self, ns: &str, mut f: F) -> Result<()>
    where
        ...
        Fut: std::future::Future<Output = Result<()>>,
    {
        ...
        let mut rows = conn
            .query(
                "SELECT from_id, to_id, strength FROM csm_associations WHERE namespace = ?1",
                params![ns],
            )
            .await
            .map_err(|e| MemoryError::database(format!("Failed to stream associations: {e}")))?;
            ...
            .map_err(|e| MemoryError::database(format!("Failed to fetch association row: {e}")))?
        {
            ...
            f(from_id, to_id, strength as f32).await?;
        }
