...
mod tests {
    ...
    fn make_absence(id: &str, query: &str, attempt_count: u32) -> AbsenceEntry {
        AbsenceEntry {
            id: id.to_string(),
    ...
    #[tokio::test]
    async fn list_absences_returns_entries_above_min_attempts() {
        let temp = NamedTempFile::new().unwrap();
        let path = temp.path().to_str().unwrap();
        let persistence = Persistence::new_local(path).await.unwrap();

        // Insert entries with attempt_count: 5, 3, 1
        let a = make_absence("absence:a", "query alpha", 5);
        let b = make_absence("absence:b", "query beta", 3);
        let c = make_absence("absence:c", "query gamma", 1);

        persistence.upsert_absence(&a).await.unwrap();
        persistence.upsert_absence(&b).await.unwrap();
        persistence.upsert_absence(&c).await.unwrap();

        // min_attempts=1 → all 3 entries returned
        let all = persistence.list_absences(1).await.unwrap();
        assert_eq!(all.len(), 3);

        // min_attempts=3 → only entries with attempt_count >= 3
        let mid = persistence.list_absences(3).await.unwrap();
        assert_eq!(mid.len(), 2);
        let ids: Vec<&str> = mid.iter().map(|e| e.id.as_str()).collect();
        assert!(ids.contains(&"absence:a"));
        assert!(ids.contains(&"absence:b"));
        assert!(!ids.contains(&"absence:c"));

        // min_attempts=5 → only the entry with attempt_count=5
        let high = persistence.list_absences(5).await.unwrap();
        assert_eq!(high.len(), 1);
        assert_eq!(high[0].id, "absence:a");

        // min_attempts=6 → nothing qualifies
        let none = persistence.list_absences(6).await.unwrap();
        assert!(none.is_empty());

        // Verify descending order by attempt_count
        let ordered = persistence.list_absences(1).await.unwrap();
        assert!(ordered[0].attempt_count >= ordered[1].attempt_count);
        assert!(ordered[1].attempt_count >= ordered[2].attempt_count);
    }
}
