...
async fn test_hnsw_filter_bug_prevention() {
    ...
    framework.inject_concept_with_metadata(
        "match",
        HVec10240::random(),
        serde_json::json!({"tag": "valid"}).as_object().unwrap().clone().into_iter().collect()
    ).await.unwrap();
    ...
    framework.inject_concept_with_metadata(
        "no-match",
        HVec10240::random(),
        serde_json::json!({"tag": "invalid"}).as_object().unwrap().clone().into_iter().collect()
    ).await.unwrap();
...
async fn test_hnsw_empty_filter_results() {
    ...
    assert!(results.is_empty(), "Results should be empty when filter matches nothing, but got {:?}", results);
}
