...
fn test_cli_parity_lifecycle() {
    ...
    cmd.args(&["--database", db_str, "inject", "c1", "--metadata", "{\"tags\":[\"test\"]}\""])
       .assert()
       .success();
    ...
    // 3. Update
    let mut cmd = Command::cargo_bin("csm").unwrap();
    cmd.args(&["--database", db_str, "update", "c1", "--metadata", "{\"tags\":[\"updated\"]}\""])
       .assert()
       .success();
...
fn test_cli_parity_graph() {
    ...
    // Associate a -> b -> c
    Command::cargo_bin("csm").unwrap()
        .args(&["--database", db_str, "associate", "a", "b", "--strength", "0.8"])
        .assert()
        .success();
    Command::cargo_bin("csm").unwrap()
        .args(&["--database", db_str, "associate", "b", "c", "--strength", "0.5"])
        .assert()
        .success();
...
fn test_cli_parity_metrics() {
    ...
    Command::cargo_bin("csm").unwrap()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Concepts Injected: 1"));
    ...
    Command::cargo_bin("csm").unwrap()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Concepts Injected: 0"));
}
...
fn test_cli_parity_probe_filtered() {
    ...
    Command::cargo_bin("csm").unwrap()
        .args(&["--database", db_str, "inject", "p1", "--metadata", "{\"cat\":\"A\"}"])
        .assert()
        .success();
    Command::cargo_bin("csm").unwrap()
        .args(&["--database", db_str, "inject", "p2", "--metadata", "{\"cat\":\"B\"}"])
        .assert()
        .success();

    Command::cargo_bin("csm").unwrap()
        .args(&["--database", db_str, "probe-filtered", "p1", "--filter", "{\"cat\":{\"$eq\":\"A\"}}"])
        .assert()
        .success()
