...
async fn persistence_roundtrip_crud() {
    ...
    persistence
        ...
        .unwrap();
    persistence
        .save_association("_default", "alpha", "alpha", 0.5)
        .await
        .unwrap();
    ...
    let associations = persistence
        .load_associations("_default", "alpha")
        .await
        .unwrap();
    assert_eq!(associations.len(), 1);

    persistence
        .delete_concept("_default", "alpha")
        .await
        .unwrap();
...
async fn persistence_rejects_association_for_missing_concept_when_fk_enabled() {
    ...
    let result = persistence
        .save_association("_default", "alpha", "missing", 0.5)
        .await;
...
async fn save_and_load_concept_preserves_ttl_and_canonical_concept_ids() {
    ...
    let loaded = persistence
        .load_concept("_default", "alpha-ttl")
        .await
        .unwrap()
        .unwrap();
...
async fn backup_and_restore_roundtrip_state() {
    ...
    persistence
        .save_concept("_default", &concept_alpha)
        .await
        .unwrap();
    ...
    persistence
        .save_concept("_default", &concept_beta)
        .await
        .unwrap();
...
async fn backup_and_restore_preserves_ttl_and_canonical_concept_ids() {
    ...
    persistence
        .save_concept("_default", &replacement)
        .await
        .unwrap();

    persistence.restore(backup_path).await.unwrap();
    let restored = persistence
        .load_concept("_default", "semantic-bridge-anchor")
        .await
        .unwrap()
        .unwrap();
...
async fn v5_namespace_migration_handles_legacy_and_prefixed_tables() {
    ...
    let loaded = persistence
        .load_concept("_default", "legacy-alpha")
        .await
        .unwrap();
    assert!(loaded.is_some());
