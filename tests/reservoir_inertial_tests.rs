...
fn test_beta_positive_changes_dynamics() {
    let mut r1 = Reservoir::new_seeded(10, 256, 42)
        .unwrap()
        .with_beta(0.0)
        .unwrap();
    let mut r2 = Reservoir::new_seeded(10, 256, 42)
    ...
    assert_ne!(
        r1.state(),
        r2.state(),
        "State with beta=0.15 should differ from beta=0.0"
    );
}
...
fn test_inertial_memory_length() {
    ...
    for i in 0..100 {
        for (j, item) in signal.iter_mut().enumerate() {
            *item = ((i + j) % 10) as f32 * 0.1;
        }
    ...
    assert!(
        sim2 > sim1,
        "beta=0.15 should retain memory longer than beta=0.0 (sim2: {}, sim1: {})",
        sim2,
        sim1
    );
}
