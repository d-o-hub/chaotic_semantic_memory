...
/// ADR-0093: background cleanup task is owned and cancelled on shutdown/drop.
#[tokio::test]
async fn ttl_cleanup_task_owned_and_shutdown() {
    use chaotic_semantic_memory::framework_ttl_advanced::TtlConfig;

    let framework = ChaoticSemanticFramework::builder()
        .without_persistence()
        .with_ttl_config(TtlConfig {
            cleanup_interval_seconds: 3600, // long interval; we only care about ownership
            ..Default::default()
        })
        .build()
        .await
    ...
    assert!(
        framework.has_ttl_cleanup_task(),
        "cleanup task must be owned when interval > 0"
    );

    framework.shutdown_ttl_cleanup().await;

    assert!(
        !framework.has_ttl_cleanup_task(),
        "shutdown must release the JoinHandle and mark cancelled"
    );
}

#[tokio::test]
async fn ttl_cleanup_absent_when_interval_zero() {
    let framework = ChaoticSemanticFramework::builder()
        .without_persistence()
        .build()
        .await
    ...
    assert!(
        !framework.has_ttl_cleanup_task(),
        "default cleanup_interval_seconds=0 must not spawn a task"
    );
}
