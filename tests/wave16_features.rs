...
fn test_bfs_cycle_does_not_loop() {
    ...
    sing.associate(NS, "b", "c", 0.9).unwrap();
    sing.associate(NS, "c", "a", 0.9).unwrap(); // back-edge

    let config = TraversalConfig::default();
    let results = sing.bfs(NS, "a", &config).unwrap();
...
fn test_bfs_disconnected_graph() {
    ...
    sing.associate(NS, "a", "b", 0.9).unwrap();
    sing.associate(NS, "c", "d", 0.9).unwrap();
...
fn test_bfs_max_results_limit() {
    ...
    sing.associate(NS, "b", "c", 0.9).unwrap();
    sing.associate(NS, "c", "d", 0.9).unwrap();
    sing.associate(NS, "d", "e", 0.9).unwrap();
...
fn test_shortest_path_cycle_terminates() {
    ...
    sing.associate(NS, "a", "b", 0.9).unwrap();
    sing.associate(NS, "b", "c", 0.9).unwrap();
    sing.associate(NS, "c", "a", 0.9).unwrap();
    sing.associate(NS, "a", "c", 0.5).unwrap(); // direct weak edge

    let config = TraversalConfig::default();
    // Must terminate and find a path
    let path = sing.shortest_path(NS, "a", "c", &config).unwrap();
...
fn test_shortest_path_hops_cycle_terminates() {
    ...
    let config = TraversalConfig::default();
    let path = sing.shortest_path_hops(NS, "a", "c", &config).unwrap();
    assert!(path.is_some());
}
...
fn test_filtered_search_subset_match() {
    ...
    for i in 0..2 {
        sing.inject(NS, make_concept_with_meta(&format!("art{i}"), "tag", "art"))
            .unwrap();
    }
